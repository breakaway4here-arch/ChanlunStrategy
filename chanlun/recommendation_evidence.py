...
def _build_summary(
    ...
    if not code:
        ...
        daily_structure.get("stale") is True
        or daily_structure.get("freshness_status") == "stale"
    ):
        summary_status = "stale"
    elif daily_structure.get("status") in {
        "stale", "conflict", "missing", "unavailable",
    }:
        summary_status = daily_structure.get("status")
    elif (
        daily_structure.get("is_final") is False
        or daily_structure.get("status") == "partial"
    ):
