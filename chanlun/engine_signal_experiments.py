...
def _should_drop_delay1_by_type_v2(point: dict, result, required_bars: int = 1) -> bool:
    if not _should_drop_delay1_by_type(point, result, required_bars=required_bars):
        return False
...
def locate_buy_sell_points_delay1_by_type_guard_v2(result):
    """Stable signal provider with type-based delay-1 guard and confirmation rescue."""
    buy_points, sell_points = locate_buy_sell_points(result)
    filtered_buy_points = _apply_filter(
        buy_points,
        lambda point: _should_drop_delay1_by_type_v2(point, result, required_bars=1),
    )
    return filtered_buy_points, sell_points
