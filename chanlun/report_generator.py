...
function buildSingleDayHistory(day, dateStr) {{
    var reports = {{}};
    reports[dateStr] = {{
        market: day.market || {{}},
    ...
    return {{ dates: [dateStr], reports: reports }};
}}
...
function renderInlineReportFallback() {{
    if (!INLINE_REPORT_DATA || INLINE_REPORT_DATA.date !== PAGE_DATE) return false;
    if (!GRANTED && DATA_BASE_PREFIX !== '') return false;
    REPORT_DATA = INLINE_REPORT_DATA;
    REPORT_DATA.date = PAGE_DATE;
    updateHeaderDate(PAGE_DATE);
    HISTORY_DATA = filterHistoryData(buildSingleDayHistory(INLINE_REPORT_DATA, PAGE_DATE), [PAGE_DATE]);
    renderAll();
    renderHistoryTabs();
    return true;
}}
...
async function init() {{
    ...
    if (window.location.protocol === 'file:') {{
        REPORT_DATA = INLINE_REPORT_DATA;
        HISTORY_DATA = buildSingleDayHistory(INLINE_REPORT_DATA, PAGE_DATE);
        updateHeaderDate(PAGE_DATE);
        renderAll();
        renderHistoryTabs();
        return;
    ...
    try {{
        ...
        var manifest = await fetchJsonWithTimeout(DATA_BASE_PREFIX + 'data.json');
        ...
        // 加载实际日期的数据
        REPORT_DATA = await fetchJsonWithTimeout(DATA_BASE_PREFIX + 'data/' + resolvedDate + '.json');
        ...
        if (GRANTED) {{
            ...
            try {{
                REPORT_DATA = await fetchJsonWithTimeout(DATA_BASE_PREFIX + 'data/' + PAGE_DATE + '.json');
                REPORT_DATA.date = PAGE_DATE;
                updateHeaderDate(PAGE_DATE);
                HISTORY_DATA = filterHistoryData(buildSingleDayHistory(REPORT_DATA, PAGE_DATE), [PAGE_DATE]);
                renderAll();
                renderHistoryTabs();
                return;
            }} catch(e2) {{
        ...
        if (renderInlineReportFallback()) return;
        renderNoPublicData();
    }}
}}
